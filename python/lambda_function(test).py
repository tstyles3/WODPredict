...
# Load movements
def load_movements(file_path="crossfit_movements.json"):
    with open(file_path, "r") as file:
        return json.load(file)

# Load athlete database
def load_athlete_database(file_path="athlete_database.json"):
    with open(file_path, "r") as file:
        return json.load(file)
...
def lambda_handler(event, context):
    ...
    for athlete_name, athlete_data in athlete_db.items():
        predictions[athlete_name] = train_model_for_athlete(athlete_name, athlete_data, all_movements)
